    ...
    GenericArray<u8, <M::Header as FixedSizeMessage>::Size>: Unpin,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        while this.header_received < this.header_buffer.len() {
            let bytes_received = ready!(Pin::new(&mut this.channel)
                .poll_read(cx, &mut this.header_buffer[this.header_received..]))
        ...
        while this.data_received < data_len {
            let bytes_received = ready!(Pin::new(&mut this.channel)
                .poll_read(cx, &mut this.data[this.data_received..data_len]))
            .map_err(ReceiveDataError::Io)?;
...
mod test {
    ...
    async fn parse_messages(
        ...
        let (reader, writer) = tokio::io::duplex(capacity as usize);
        let (reader, mut writer) = (reader.compat(), writer.compat_write());
    ...
    use crate::messages::{DataMsg, FixedSizeMessage};
